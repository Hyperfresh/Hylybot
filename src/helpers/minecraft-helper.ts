...
export async function updateMinecraft(db: Db, user: string) {
    ...
    if (!value) {
        ...
        if (result.gametags.mc == null) {
            await db.collection("profiles").updateOne({user: user}, {$set: {"gametags.mc": null}})
        }
        return -1
    }
    ...
    await db.collection("profiles").updateOne({user: user}, {$set: {"gametags.mc": mcUser.uuid}})
    return 0
}
...
export async function parseMinecraft(uuid: string) {
    ...
    try {
        let document: any = load(fs.readFileSync(`${userData}/${uuid}.yml`, "utf8"))
