...
export default async function birthdayCheck(db: Db, bot: Client) {
    ...
    if (hh == 7) {
        ...
        let data = await db.collection('profiles').find({ 'bday': { $eq: todayString } }).toArray();

        if (data.length >= 1) {
            console.log(`Found ${data.length}:`, data)
            let channel: any = guild.channels.cache.find(val => val.id == config.ANNOU_ID)
            ...
            data.forEach(async user => {
                ...
                channel.send({content: `**Hey @everyone - go wish <@!${userInfo.id}> happy birthday!** 🎉🎉`, embeds: [embed]})
            });
