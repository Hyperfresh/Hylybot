...
module.exports.run = {
    ...
    .addUserOption(option => option.setName("user").setDescription("Whose birthday do you want to check?").setRequired(false)),
    async execute(interaction: CommandInteraction, db: Db) {
        ...
        if (!result || result.bday == "Unknown") {
            ...
            if (user.id == interaction.user.id) interaction.followUp({content: "To register your birthday, use `/profile edit bday`.", ephemeral: true})
            return
        }
        ...
        if (now.monthShort == "Dec" && bdayArray[1] != "Dec") bdayParse = `${bdayArray[0]} ${bdayArray[1]} ${now.year + 1}`
        ...
        if (length.toMillis() < 0 && length.toMillis() >= -604800000) {
            ...
            if (length.toMillis() >= -86400000) {
                ...
                birthday = `was ${split[0] * -1} days, ${split[1]* -1} hours and ${split[2] * -1} minutes ago.`
            }
        }
